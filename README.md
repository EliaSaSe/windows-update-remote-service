...
The projects consists of two parts, an agent that's needed to be installed on the Windows servers, and a [WPF] client application.

The client can connect to agents and send commands like "search", "select update", "download", "install" and "reboot". The agents pushes progress and state changes back to all connected clients.
...
When the agent starts, It creates an [update session] to communicate with Windows Update. Operations in an update sessions are isolated from operations in other update sessions. The Windows Update menu of the control panel uses it's own update session. That means, that the activity of the agent is not shown in the Windows Update menu of the control panel. Nevertheless, Windows Updates handles concurrency. Only one session at time is able to install updates. The agent reacts to such situations and reports that the commanded operation is not possible due activity of other sessions. Updates that are downloaded by other sessions will not be redownloaded, all sessions are using the same "update storage".

### State machine

The [Windows Update Agent API] protects the agent from doing invalid actions but also needs to be interfaced correctly. The agent manages an internal state machine to implement a valid patch flow. The client can use commands to transit the state. Invalid transitions will be rejected. Some transitions occurs automatically based on events in the Windows Update session.
...
"Windows security" is used, depending of the host configuration, Kerberos or NTLM authentication will be performed. The client will be authenticated with the Windows user context, in which they is running. The agent rejects any user that does not have local administrator privileges on its host. So an Active Directory user with this permissions on all hosts that needs to managed, is required. This autorisation behavior is currently hardcoded.
...
The agent is designed as long running process with a less footprint on the host system. While the agent it self does not consume many CPU cycles, remember that Windows Update can do, when an action is commanded.
...
| createFirewallRule | When enabled, the agent tries to register it self in the windows firewall and opens the configured port for its binary. On shutdown, the agent tries to remove this rule. |
...
Here is an PowerShell example to register the agent as Windows service. The service will be executed with ``LocalSystem``.
...
The agent was only tested with ``LocalSystem`` privileges. I never tried out the least privileges requirements. If you want to try, this are my suggestions to start with:
...
Unpack the zip and copy the content to the desired location, then execute ``WcfWuRemoteClient.exe``. When you get "access denied" messages when conntecting to agents despite local administrator privileges on the target hosts, start the client with evaluated privileges. 

## Getting started

1. For a first entrance on your local machine, just unapck the zip and start ``WcfWuRemoteClient.exe`` and ``WcfWuRemoteService.exe``. You need local administrator privileges and you may need to start the proccess with right click and "Run as administrator" for full evaluated privileges.
